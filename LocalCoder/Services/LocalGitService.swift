...
final class LocalGitService: @unchecked Sendable {
    ...
    func changedFiles() async throws -> [ChangedFile] {
        let path = self.localURL.path
        ...
        return try await Task.detached {
            ...
            var statusList: OpaquePointer?
            defer { if let statusList { git_status_list_free(statusList) } }
            try git2Check(git_status_list_new(&statusList, repo, &statusOpts), context: "Get status")
            ...
            for i in 0..<count {
                ...
                if let diff = entry.pointee.head_to_index {
                    let filePath = String(cString: diff.pointee.new_file.path)
                    let status: ChangedFile.ChangeStatus
                    if s.rawValue & GIT_STATUS_INDEX_NEW.rawValue != 0 { status = .added }
                    else if s.rawValue & GIT_STATUS_INDEX_MODIFIED.rawValue != 0 { status = .modified }
                    else if s.rawValue & GIT_STATUS_INDEX_DELETED.rawValue != 0 { status = .deleted }
                    else if s.rawValue & GIT_STATUS_INDEX_RENAMED.rawValue != 0 { status = .renamed }
                    else { status = .modified }
                    files.append(ChangedFile(path: filePath, status: status, staged: true))
                }
                ...
                if let diff = entry.pointee.index_to_workdir {
                    let filePath = String(cString: diff.pointee.new_file.path)
                    let status: ChangedFile.ChangeStatus
                    if s.rawValue & GIT_STATUS_WT_NEW.rawValue != 0 { status = .untracked }
                    else if s.rawValue & GIT_STATUS_WT_MODIFIED.rawValue != 0 { status = .modified }
                    else if s.rawValue & GIT_STATUS_WT_DELETED.rawValue != 0 { status = .deleted }
                    else if s.rawValue & GIT_STATUS_WT_RENAMED.rawValue != 0 { status = .renamed }
                    else { status = .modified }
                    files.append(ChangedFile(path: filePath, status: status, staged: false))
                }
            }
    ...
    func stageAll() async throws -> Int {
        let path = self.localURL.path
    ...
    func commit(message: String, authorName: String, authorEmail: String) async throws -> String {
        let path = self.localURL.path
        ...
        return try await Task.detached {
            ...
            var parentCommit: OpaquePointer?
            defer { if let parentCommit { git_commit_free(parentCommit) } }
            try git2Check(git_commit_lookup(&parentCommit, repo, &headOidCopy), context: "Lookup HEAD commit")
            ...
            var sig: UnsafeMutablePointer<git_signature>?
            defer { if let sig { git_signature_free(sig) } }
            try git2Check(git_signature_now(&sig, authorName, authorEmail), context: "Create signature")
    ...
    func push(pat: String) async throws {
        let path = self.localURL.path
        ...
        try await Task.detached {
            ...
            var pushRemote: OpaquePointer?
            defer { if let pushRemote { git_remote_free(pushRemote) } }
            try git2Check(git_remote_lookup(&pushRemote, repo, "origin"), context: "Lookup origin")
    ...
    // MARK: - Write a file into the repo working directory

    func writeFile(relativePath: String, content: String) throws {
        let fileURL = localURL.appendingPathComponent(relativePath)
    ...
    /// Delete a file from the repo working directory
    func deleteFile(relativePath: String) throws {
        let fileURL = localURL.appendingPathComponent(relativePath)
        try FileManager.default.removeItem(at: fileURL)
    }

    /// Check if a file exists in the repo working directory
